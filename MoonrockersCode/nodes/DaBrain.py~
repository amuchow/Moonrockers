...
StartMiningDistance      = 205
StopMiningDistance       = 282
...
ForwardAdjustOffsetState      = 50
ForwardAdjustOffsetLeftState  = 51 
ForwardAdjustOffsetRightState = 52
ForwardDriveTimeState  = 53
...
LeftForwardRightBackwardMYSPIN = "300100001102"
RightForwardLeftBackwardMYSPIN = "300100010012"


StateOneZero    = "300100010102"
StateOneOne     = "300100000102"
StateOneTwo     = "300100010002"
#Mining
StateTwoZero    = "300000110102"
StateTwoOne     = "301000000102"
StateTwoTwo     = "301000010002"
...
class Gui(object):
    ...
    def update(self):
        ...
        #STATE NUMBER 50
        if self.STATE == ForwardAdjustOffsetState:
            canvas.itemconfig(self.StateOneRectangle, outline = "#FF0000")
            canvas.itemconfig(self.stateGoalText, text = "Pick Left or Right" )
            self.ForwardAdjustOffset()

        #STATE NUMBER 51
        if self.STATE == ForwardAdjustOffsetLeftState:
            canvas.itemconfig(self.StateOneRectangle, outline = "#FF0000")
            canvas.itemconfig(self.stateGoalText, text = "Do for .25s" )
            self.ForwardAdjustOffsetLeft()

        #STATE NUMBER 52
        if self.STATE == ForwardAdjustOffsetRightState:
            canvas.itemconfig(self.StateOneRectangle, outline = "#FF0000")
            canvas.itemconfig(self.stateGoalText, text = "Do for .25s" )
            self.ForwardAdjustOffsetRight()

        #STATE NUMBER 53
        if self.STATE == ForwardDriveTimeState:
            canvas.itemconfig(self.StateOneRectangle, outline = "#FF0000")
            canvas.itemconfig(self.stateGoalText, text = "Drive Forward for 5 seconds" )
            self.ForwardDriveTime()
    ...
    def SpinControl(self):
        ...
        if time_spent > .50 and self.TagSeen == 0:
            #Rotate State
            self.STATE = StartSpinState
            self.time = time.time()    
    ...
    def StartSpin(self):
        ...
        self.s.send(RightForwardLeftBackwardMYSPIN)
    ...
    def TraversingOut(self):
        ...
        time_spent = time.time() - self.time
        if time_spent > 1.0 and self.Z > 60:
            #Angle too big.
            ...
            if (self.DistX > 6 or self.DistX < -6) and self.Angle != -999:
                #Hello
                rospy.loginfo("Forward OFFSET CORRECTION!")
                self.STATE = ForwardAdjustOffsetState
                self.time = time.time()    
                rospy.loginfo("Robot is forward adjusting the Offet Now")
                return
    ...
    def Mining(self):
        ...
        #Last Rear Conveyor Shift
        if (self.Z > adjustedMiningDistance - 4)  and self.ConveyorFour == 1:
            self.STATE = RearConveyorState
            self.ConveyorFour   = 0
            self.time = time.time()
            return
        #First Rear Conveyor Shift
        if (self.Z > adjustedMiningDistance - 21)  and self.ConveyorOne == 1:
            self.STATE = RearConveyorState
            self.ConveyorOne   = 0
            self.time = time.time()
            return
        ...
        if (self.Z > adjustedMiningDistance - 42)  and self.ConveyorTwo == 1:
            self.STATE = RearConveyorState
            self.ConveyorTwo   = 0
            self.time = time.time()
            return
        ...
        if (self.Z > adjustedMiningDistance - 63)  and self.ConveyorThree == 1:
            self.STATE = RearConveyorState
            self.ConveyorThree   = 0
            self.time = time.time()
            return
    ...
    #State 50
    def ForwardAdjustOffset(self):
        rospy.loginfo("ForwardAdjust Offset State") 
        #Finished Driving Out
        if self.Z > (StartMiningDistance - RobotSize) :
            self.STATE = LowerConveyorState
            self.time = time.time()
        ...
        if self.DistX < 0:
            #Rotate LeftFace State
            self.STATE = ForwardAdjustOffsetLeftState
            self.time = time.time()    
            rospy.loginfo("Robot is ForwardOffsetLeft now. STATE change to AdjustOffsetLeft(21)")
            return
        else:
            #Rotate RightFace State
            self.STATE = ForwardAdjustOffsetRightState
            self.time = time.time()    
            rospy.loginfo("Robot is ForwardoffsetRight now. STATE change to RotateFaceRight(20)")
            return
        

    #State 51
    def ForwardAdjustOffsetLeft(self):
        rospy.loginfo("Adjust Offset Left State") 
        #Finished Driving Out
        if self.Z > (StartMiningDistance - RobotSize) :
            self.STATE = LowerConveyorState
            self.time = time.time()
            #rospy.loginfo("Robot has NEEDS to go mine now")
            return
        ...
        if time_spent > .5:
            self.STATE = ForwardDriveTimeState
            self.time = time.time()    
            rospy.loginfo("Robot has Forward adjusted left offset. STATE change to drivebacktime(28)")
            return

    #State 52
    def ForwardAdjustOffsetRight(self):
        rospy.loginfo("Adjust Offset Right State") 
        #Finished Driving Out
        if self.Z > (StartMiningDistance - RobotSize) :
            self.STATE = LowerConveyorState
            self.time = time.time()
            #rospy.loginfo("Robot NEEDS to go mine now")
            return
        ...
        if time_spent > .5:
            self.STATE = ForwardDriveTimeState
            self.time = time.time()    
            rospy.loginfo("Robot has Forward adjusted right offset. STATE change to drivebacktime(28)")
            return

    #State 53
    def ForwardDriveTime(self):

        rospy.loginfo("Drive Forward Time State") 
        #Finished Driving Out
        if self.Z > (StartMiningDistance - RobotSize) :
            self.STATE = LowerConveyorState
            self.time = time.time()
        ...
        if time_spent > 3.5:
            self.STATE = AngleAdjustForwardState
            self.time = time.time()    
            rospy.loginfo("Robot has driven forward time STATE change to TraversingOut(7)")
            return
    ...
    def callback(self, data):
        ...
        if (data.data[1] > 300 or data.data[1] < 10) or (data.data[0] < -150 or data.data[0] > 150):
            rospy.loginfo("THIS DATA IS GOING IN THE TRASH BECAUSE I SAY SO")
            return
        self.TagSeen = 1
        if self.changeCounter < OutlierDataThreshold:
            if self.X != -1 and self.Z != -999:
                ...
                if xChangePercent > AllowableChangeThreshold and  xChange > 30:       
                    rospy.loginfo("X Changed too rapidly, ignoring outlier.")
                    #Increment the rejected data counter
                    self.changeCounter += 1
                    return
                
                elif zChangePercent > AllowableChangeThreshold and zChange > 30:   
